  ...
  try {
    ...
    switch (op) {
      ...
      case 'getNotifications':
        result = getNotifications_API(verifiedUser);
        break;
...
function getNotifications_API(user) {
  if (!user || !user.name) {
    return [];
  }
  ...
  const helpAssigneeCol = headers.indexOf('HelpAssignee');
  const deadlineCol = headers.indexOf('Deadline');
  const createdCol = headers.indexOf('Created At');
  const taskCol = headers.indexOf('Task');
  const projectCol = headers.indexOf('ProjectID');
  const statusCol = headers.indexOf('Status');
  const idCol = headers.indexOf('_id');

  if (ownerCol === -1 || deadlineCol === -1 || taskCol === -1 || projectCol === -1 || statusCol === -1 || idCol === -1) {
    Logger.log('One or more required columns are missing in Tasks sheet for notifications.');
    return [];
  }
  ...
  if (projectsSheet) {
    ...
    projectData.shift(); // remove headers
    projectData.forEach(row => {
      projectNames[row[0]] = row[1]; // ProjectID -> Project Name
    });
  }
  ...
  for (let i = allData.length - 1; i >= 0; i--) {
    const row = allData[i];
    const isOwner = normalizeString(row[ownerCol]) === normalizedUserName;
    const isHelper = normalizeString(row[helpAssigneeCol]) === normalizedUserName;

    if (isOwner || isHelper) {
      const deadline = row[deadlineCol] ? new Date(row[deadlineCol]) : null;
      const createdAt = row[createdCol] ? new Date(row[createdCol]) : null;
      const status = row[statusCol];

      const isNotDone = status !== 'Done' && status !== 'Cancelled';

      if (isNotDone && deadline && deadline >= now && deadline <= sevenDaysFromNow) {
        notifications.push({
          type: 'deadline',
          taskName: row[taskCol],
          projectName: projectNames[row[projectCol]] || row[projectCol],
    ...
    if (notifications.length >= 40) { // A bit larger to allow for deduplication
      break;
    }
  }

  const uniqueNotifications = Array.from(new Set(notifications.map(n => n._id)))
    .map(id => {
