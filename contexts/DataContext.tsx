...
interface DataContextType {
  ...
  createProject: (projectName: string, selectedTasks: string[]) => Promise<void>;
  createTask: (newTaskData: Omit<Task, 'rowIndex' | '_id' | 'Check'>) => Promise<void>;
...
export const DataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const formatAndSortTasks = useCallback((data: any[]): Task[] => {
  ...
  const fetchTasks = useCallback(async (projectId: string) => {
    ...
    try {
      const res = await fetch(`${SCRIPT_URL}?op=getTasks&projectId=${projectId}`);
  ...
  const createProject = useCallback(async (projectName: string, selectedTasks: string[]) => {
  ...
  const createTask = useCallback(async (newTaskData: Omit<Task, 'rowIndex' | '_id' | 'Check'>) => {
  ...
  const value = {
    projects, tasks, initialTasks, selectedProjectId, loadingMessage, error,

    filteredTasks, operationScore, efficiencyRatio, onTimePerformance, tasksByStatus, tasksByOwner,

    setSelectedProjectId, handleProjectSelect, saveTask, createProject, createTask, confirmDelete
  };
